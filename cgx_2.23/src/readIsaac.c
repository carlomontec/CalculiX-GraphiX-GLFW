...
int calcAddEntitiesIsaac( Summen *anz, Nodes *node, Elements *elem, Datasets *lcase, int elem_type, double PINF, double TINF, double R_GAS, double kappa, double cpg )
{
  int lc, n, i, j, ncomps;

  double p0, t0, rho, ma, mau, mav, maw, rhoE, pcalc, ps, pt, ts, tt, a, vu, vv, vw, v;
  double rho0, rhocalc;
  ...
  printf(" calculate additional entities based on: TINF:%f PINF:%f R_GAS:%f GAMMA:%f\n",TINF,PINF,R_GAS,kappa );
	...
	*/
        ...
        for(i=0; i<anz->n; i++)
	{
          ...
          else
	  {
          ...
          /* in ISAAC : (1./kappa)==PREF; 1.=RHOINF; pps=pcalc; */
...
int readIsaac(char *datin, Summen *anz, Nodes **nptr, Elements **eptr, Datasets **lptr, NodeBlocks **bptr, int elem_type )
{
  ...
  int i, j, k, n, ld100, lfile, print;
  char meshfile[MAX_LINE_LENGTH];
  char rec_str[MAX_LINE_LENGTH];
  char buffer[MAX_LINE_LENGTH];
  char dat[3][MAX_LINE_LENGTH];
  int  anz_n;
  int  e_nmax=1, e_nmin=1;
  int  length;
  double fval[1];
  double MACH=0., PINF=0., TINF=0., R_GAS=_R_GAS, GAMMA=1.4, LREF=1., RE=1e6, PR=0.7, VU=0.,VV=0.,VW=0.;
  double vel,wl,dynVisc,cpGas;
  ...
  do
  {
    length = frecord( handle2, rec_str);
    if (rec_str[length] == (char)EOF) break;
    dat[2][0]=0;
    length= sscanf(rec_str,"%s %s %s",dat[0], dat[1], dat[2]);
    print=1;
    if (compareStrings(dat[0],"'MACH'")>0) MACH=atof(dat[1]);
    else if (compareStrings(dat[0],"'U'")>0) VU=atof(dat[1]);
    else if (compareStrings(dat[0],"'V'")>0) VV=atof(dat[1]);
    else if (compareStrings(dat[0],"'W'")>0) VW=atof(dat[1]);
    else if (compareStrings(dat[0],"'RE'")>0) RE=atof(dat[1]);
    else if (compareStrings(dat[0],"'PR'")>0) PR=atof(dat[1]);
    else if (compareStrings(dat[0],"'TINF'")>0) TINF=atof(dat[1]);
    else if (compareStrings(dat[0],"'GAMMA'")>0) GAMMA=atof(dat[1]);
    else if (compareStrings(dat[0],"'REFERENCE")>0) LREF=atof(dat[2]);
    else print=0;
    if(print)  printf ("%s %s %s\n",dat[0], dat[1], dat[2] );
  } while(1);
  
  anz->l++;
  if(VU||VV||VW)
  {
    printf("found jet conditions, used now for PINF calculation\n");
    vel=sqrt(VU*VU+VV*VV+VW*VW)*sqrt(GAMMA*R_GAS*TINF);
  }
  else vel=MACH*sqrt(GAMMA*R_GAS*TINF);
  cpGas=R_GAS/(1.-(1./GAMMA));
  wl=0.0245*(1.+0.00225*(TINF-273.15));
  dynVisc=PR/cpGas*wl;
  PINF=RE*(R_GAS*TINF*dynVisc)/(LREF*vel);
  printf("v:%f cp:%f wl:%f dynVisc:%f PINF:%f\n",vel,cpGas,wl,dynVisc,PINF);

  /* calculate additional entities */
  calcAddEntitiesIsaac( anz, node, elem, lcase, elem_type, PINF, TINF, R_GAS, GAMMA, cpGas);
