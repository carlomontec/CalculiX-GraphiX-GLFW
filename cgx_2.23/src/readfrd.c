...
int readfrd( char *datin, Summen *anz, Nodes **nptr, Elements **eptr, Datasets **lptr, int read_mode )
{
  ...
  while(length)
  {
    ...
    if(strlen(rec_str)>73) format_flag = stoi(rec_str,74,75);
    else format_flag = 0;
    ...
    else if(flag == 100)
    {
	  ...
	  {
	    ncomps=0;
	    for(i=0; i<lcase[anz->l].ncomps; i++) if(lcase[anz->l].iexist[i]!=1) ncomps++;
            ...
            for(n=0; n<nod_data; n++)
            {
              ...
              length=fread((float *)value,sizeof(float),ncomps,handle);
	      // printf("n:%d N:%d ",n+1, nodenr);
	      k=0;
              for(i=0; i<lcase[anz->l].ncomps; i++) if(lcase[anz->l].iexist[i]!=1)
              {
	        // printf(" %f",value[k]); 
                lcase[anz->l].dat[i][nodenr]= value[k];
		k++;
              }
              // printf("\n");
            }
...
{
  int i,j, n;
  int  length, flag, format_flag, nodenr=0, ncomps, nvals;
    ...
    do
    {
      ...
      if ( format_flag>=2)
      {
        ...
        if ( format_flag==2)
        {
          ...
          {
            length=fread((int *)&nodenr,sizeof(int),1,handle);
            length=fread((float *)value,sizeof(float),nvals,handle);
  	  // printf("N:%d ",nodenr);
            for(i=0; i<lcase[lc].ncomps; i++)
            {
  	    // printf(" %f",value[i]); 
              if(lcase[lc].iexist[i]!=1) lcase[lc].dat[i][nodenr]= value[i];
	      else lcase[lc].dat[i][nodenr]=0.;
            }
  	  // printf("\n");
          }
        }
        if ( format_flag==3)
        {
          ...
          {
            length=fread((int *)&nodenr,sizeof(int),1,handle);
            length=fread((double *)dvalue,sizeof(double),nvals,handle);
  	  // printf("N:%d ",nodenr);
            for(i=0; i<lcase[lc].ncomps; i++)
            {
  	    // printf(" %f",dvalue[i]); 
              if(lcase[lc].iexist[i]!=1) lcase[lc].dat[i][nodenr]= dvalue[i];
	      else lcase[lc].dat[i][nodenr]=0.;
            }
  	  // printf("\n");
          }
  ...
  *vptr=dat;
  ...
  if(lcase[lc].format_flag==2)
  {
    ...
    do
    {
      ...
      length+=fread((float *)value,sizeof(float),nvals,handle)*sizeof(float);
      //printf("N:%d ",inodenr);
      //for(i=0; i<nvals; i++) printf(" %f",value[i]);
      //printf("\n");
      if(inodenr==nodenr) break;
      else offset+=length;
    }while(++n<lcase[lc].numnod);
    for(i=0; i<nvals; i++)
    {
	//printf(" %f",value[i]);
      dat[i]= value[i];
  ...
  if(lcase[lc].format_flag==3)
  {
    ...
    {
      length=fread((int *)&inodenr,sizeof(int),1,handle)*sizeof(int);
      length+=fread((double *)dvalue,sizeof(double),nvals,handle)*sizeof(double);
      //printf("N:%d ",inodenr);
      //for(i=0; i<nvals; i++) printf(" %f",dvalue[i]);
      //printf("\n");
      if(inodenr==nodenr) break;
      else offset+=length;
    }while(++n<lcase[lc].numnod);
