...
\usepackage{color}
\usepackage{hyperref}
\providecommand{\htmlref}[2]{\hyperref[#2]{#1}}
\begin{document}
\title{CalculiX USER'S MANUAL\\ - CalculiX GraphiX, Version 2.23 -\\ {\large CalculiX GraphiX (GLFW Edition)}}
\author{Klaus Wittig\\ {\small Extended and Modernized for GLFW Edition by Carlo Monjaraz-Tec}}
\maketitle
\begin{figure}[h]
\includegraphics[width=12cm]{jetengine.pdf}
...
\section{Introduction}
 This document is the description of CalculiX GraphiX (cgx). This program is designed to generate and display finite elements (FE) and results coming from CalculiX CrunchiX (ccx). If you have any problems using cgx, this document should solve them. If not, you might send an email to the author \cite{cgx}.

\paragraph{CalculiX GraphiX (GLFW Edition) Extension Notice:}
This edition of the manual is an extension of the original CalculiX GraphiX documentation by Klaus Wittig. It incorporates the enhancements of the modern \textbf{CalculiX GraphiX (GLFW Edition)}, which replaces legacy X11/GLUT dependencies with native GLFW3 windowing across macOS, Linux, and Windows, integrates vector typography (\texttt{stb\_truetype}), an interactive in-window command bar with fuzzy suggestions, standard CAD/Blender navigation gestures, default Dark Mode and 3D Perspective projection, standard axonometric viewpoints (Isometric, Dimetric, Trimetric), direct PNG and MP4/GIF hardcopy captures without ImageMagick, embedded in-memory TetGen 1.6.0 meshing, and native ParaView VTU/PVD export. Features and capabilities introduced in the GLFW Edition are annotated throughout this manual with footnotes.

The Concept and File Format sections give some background on functionality and mesher capabilities. The Getting Started section describes how to run the verification examples you should have obtained along with the code of the program. You might use this section to check whether you installed CalculiX correctly. Then, a detailed overview is given of the menu and all the available keywords in alphabetical order in the Menu and Commands sections respectively. Finally, the User's Manual ends with the appendix and some references used while writing the code.


\section{Concept}
This program uses modern OpenGL for hardware-accelerated 3D visualization and the GLFW3 library \cite{glfw}\footnote{In CalculiX GraphiX (GLFW Edition), legacy GLUT/GLX/X11 dependencies are replaced by GLFW3, enabling native windowing on macOS (Cocoa), Linux (Wayland/X11), and Windows.} for window management and event handling. Anti-aliased vector typography is powered by the embedded \texttt{stb\_truetype} engine \cite{stb}\footnote{Typography engine introduced in CalculiX GraphiX (GLFW Edition) with calibrated 3-tier font sizing (Small, Medium, Big) and dynamic High-DPI/Retina display scaling.}.

The cgx has pre- and post-processor capabilities. It is able to generate and display beam, shell and brick elements in its linear and quadratic form (fig. \ref{cover}), tets can be generated from within cgx via the embedded in-memory TetGen 1.6.0 engine\footnote{CalculiX GraphiX (GLFW Edition) vendors TetGen 1.6.0 directly into the executable using in-memory C++ API calls without disk temporary files.} \cite{TETGEN} or the program ng\_vol (part of NETGEN \cite{NETGEN}) if accessible (see also ''\htmlref{How to deal with CAD-geometry}{How to deal with CAD-geometry}''). TETGEN comes with cgx.
...
\end{itemize}
The following file-formats are available to write a mesh and certain boundary-conditions or results:
\begin{itemize}
...
\item vtu (VTK XML UnstructuredGrid) \cite{paraview}\footnote{VTU mesh and result exporter with master ParaView collection (.pvd) generation introduced in CalculiX GraphiX (GLFW Edition).}, writes meshes and static or transient result fields directly to ParaView in fast inline Base64 binary encoding (or optional ASCII).
\end{itemize}
...
The mouse is used to manipulate the view-point and scaling of the object inside the drawing area (figure \ref{mainwindow}). Rotation of the object is controlled by the left mouse button (smooth trackball rotation around the model center), zoom in and out by the middle mouse button (or mouse scroll wheel / trackpad pinch\footnote{Scroll wheel / trackpad pinch stepped zoom and unbounded zoom range introduced in CalculiX GraphiX (GLFW Edition).}), and translation of the object is controlled by the right mouse button (drag threshold $\ge 4\text{px}$).

In addition, modern CAD and Blender keyboard/mouse navigation modifiers are supported\footnote{CAD and Blender navigation modifiers and right-click tap vs. drag context menu discrimination introduced in CalculiX GraphiX (GLFW Edition).}:
\begin{itemize}
\item \textbf{Left Click + Drag}: Rotate / Orbit the 3D model around its center.
\item \textbf{Shift + Left Drag} or \textbf{Ctrl/Cmd + Left Drag}: Pan / Translate the model horizontally and vertically.
\item \textbf{Alt / Option + Left Drag}: Smooth continuous dynamic zoom.
\item \textbf{Scroll Wheel / Trackpad Pinch}: Stepped continuous zoom.
\item \textbf{Right Click + Drag}: Pan / Translate (when dragged $\ge 4\text{px}$).
\item \textbf{Right Click Tap} (release without drag): Opens the multi-level cascading context menu.
\end{itemize}

Inside the menu area, the mouse triggers the \htmlref{main menu}{Menu} with the left button.\\\\In addition the mouse controls the \htmlref{animation}{animation} of nodal values. The animation will stop if the mouse pointer is not in the drawing area but will start again if the pointer enters the drawing area. This can be prevented by pressing the middle mouse button while the mouse pointer is in the menu area.  Pressing the right button will release the next frame. A frozen animation can be released by pressing the middle button. The previous frame can be reloaded by pressing the middle mouse button twice and the right button once (while the mouse is in the menu area).
  
\begin{figure}[h]
\includegraphics[width=12cm]{mainwindow.pdf}
\caption{\label{mainwindow}structure of the main-window}
\end{figure}

\subsection{\label{Keyboard}Keyboard}
The Keyboard is used for command line input and specifying the type of entities when selecting them with the mouse pointer. The command line is preferable in situations where pure mouse operation is not convenient (i.e. to define a certain value) or for batch controlled operations. Therefore most commands are only available over the command line.

In addition to echoing in the parent terminal, an interactive in-window command bar is embedded directly at the bottom of the graphics viewport\footnote{In-window interactive command bar with command history, multi-line clipboard paste, and Levenshtein fuzzy command suggestion engine introduced in CalculiX GraphiX (GLFW Edition).}. The user can click the bottom bar or simply type while the mouse is in the graphics window. The command bar includes:
\begin{itemize}
\item \textbf{Command History}: Browse previous commands using the \texttt{ARROW\_UP} and \texttt{ARROW\_DOWN} keys.
\item \textbf{Smart Fuzzy Typo Suggestions}: If an unrecognized command is typed, an embedded Levenshtein distance matcher automatically suggests the closest valid CGX command (e.g. \texttt{fram} $\rightarrow$ \texttt{frame}) with informative status feedback displayed directly in the bar for 6 seconds.
\item \textbf{Clipboard Support}: Multi-line clipboard paste and standard editing shortcuts are supported.
\item \textbf{Visibility Toggle}: The command bar can be toggled via the \textbf{GUI Settings} $\rightarrow$ \textbf{Toggle Command Line Bar} menu or with the command ``\htmlref{view}{view} cl''.
\end{itemize}

The following special keys are used:
\begin{verbatim}
Special Keys:
 ARROW_UP:   previous command (command history)
 ARROW_DOWN: next command (command history)
...
The main menu pops up when tapping the right mouse-button (or pressing the left mouse-button inside the menu-area in legacy mode, see figure \ref{mainmenu})\footnote{Cascading popup context menu triggered at the mouse cursor position via right-click tap introduced in CalculiX GraphiX (GLFW Edition).}. It should be noted that there are equivalent command-line functions for most of the menu-functions. This can be used for batch-controlled post-processing (see command ''\htmlref{view}{view}''). Next the entities inside the main menu will be explained:
  
\begin{figure}[h]
\includegraphics[width=12cm]{mainmenu.pdf}
\caption{\label{mainmenu}structure of the main-menu}
\end{figure}

\subsection{\label{GUI Settings}GUI Settings}
\footnote{Dedicated top-level submenu introduced in CalculiX GraphiX (GLFW Edition) for rapid display and typography configuration.}
This menu entry provides direct access to modern viewport and interface controls:
\begin{itemize}
\item \textbf{Toggle Dark Mode}: Switches the viewport background between the signature dark slate aesthetic (\texttt{\#0D121A}) and classic clean white.
\item \textbf{Toggle Perspective 3D}: Switches between true 3D perspective projection and orthographic (parallel) projection.
\item \textbf{Toggle Command Line Bar}: Hides or shows the interactive in-window command bar at the bottom of the viewport.
\item \textbf{[Experimental] Modern GPU Pipeline}: Toggles the hardware-accelerated 3D rendering pipeline using packed Vertex Buffer Objects (VBOs) and custom GLSL shaders with metallic Blinn-Phong surface lighting and GPU-sampled colormaps (1 single draw call for the entire model). When disabled, the engine falls back to legacy OpenGL display lists.
\item \textbf{Text Size}: Adjusts the vector typography scale across three calibrated tiers:
  \begin{itemize}
  \item \textbf{Small (14pt)}: Compact size for dense model displays and low-resolution screens.
  \item \textbf{Medium (20pt / Default)}: Standard reading size optimized for modern monitors.
  \item \textbf{Big (32pt)}: Enlarged typography ideal for 4K / High-DPI screens and presentations.
  \end{itemize}
\end{itemize}
...
With this option, it is possible to switch between the default dark mode background and a white background.
...
Triggers the display of a ruler bar. In the GLFW Edition\footnote{Stationary 2D HUD ruler anchored in screen space introduced in CalculiX GraphiX (GLFW Edition).}, the ruler is pinned to the bottom-right corner in a dedicated 2D orthographic HUD overlay with full OpenGL state isolation, remaining stationary during 3D model rotation.

\subsubsection{\label{Toggle Logarithmic Scale}Toggle Logarithmic Scale}
Toggles the results color mapping between a standard linear scale and a base-10 logarithmic scale ($\log_{10}$). In the GLFW Edition\footnote{ParaView-compatible logarithmic color scaling introduced in CalculiX GraphiX (GLFW Edition) with automatic positive floor clamping and base-10 power label formatting ($10^x$).}, selecting logarithmic scale applies a continuous $\log_{10}$ transfer function. If the field results contain zeros or negative values, the lower bound is automatically clamped to a positive floor ($\max(v_{\text{pos,min}}, v_{\max} \times 10^{-4})$), non-positive values are mapped to the bottom color band, and the 2D legend bar formats decade ticks in clear mathematical power notation ($10^1, 10^2, 10^3$).

\subsubsection{\label{Colormap}Colormap}
Changes the colormap of the color scale. In the GLFW Edition\footnote{Perceptually uniform \texttt{cubehelix (reversed)} default colormap with lifted graphite floor and modern palettes introduced in CalculiX GraphiX (GLFW Edition).}, the default colormap is \texttt{cubehelix (reversed)} (designed by D. A. Green \cite{cubehelix}, optimized for metallic 3D surface shading and monochrome print clarity). Available palettes include \texttt{cubehelix (reversed)}, \texttt{cubehelix}, \texttt{coolwarm}, \texttt{turbo}, \texttt{viridis}, \texttt{inferno}, \texttt{jet}, \texttt{classic}, and \texttt{gray}. See also ''\htmlref{cmap}{cmap}''.
...
In CalculiX GraphiX (GLFW Edition)\footnote{In CalculiX GraphiX (GLFW Edition), standard axonometric viewpoints (Isometric, Dimetric, Trimetric) are available alongside the principal Cartesian axis views.}, the orientation menu sets the camera view direction.
...
To rotate into standard isometric view (yaw $45^\circ$, pitch $35.264^\circ$).
\subsubsection{Dimetric}
To rotate into standard dimetric view (yaw $26.565^\circ$, pitch $19.471^\circ$).
\subsubsection{Trimetric}
To rotate into standard trimetric view (yaw $30^\circ$, pitch $20^\circ$).

\subsection{\label{Hardcopy}Hardcopy}
In CalculiX GraphiX (GLFW Edition)\footnote{In CalculiX GraphiX (GLFW Edition), screenshot captures and movie recordings are written directly from the OpenGL framebuffer to PNG, MP4, and GIF without requiring ImageMagick.}, screenshots and animation movies can be recorded directly from the viewport. To create a hard-copy during animation, it is useful to stop the animation first with the middle mouse button while inside the menu area of the main window and then release one picture after the other with the right button until the desired amplitude or step is reached.
 
\subsubsection{\label{Save PNG Screenshot}Save PNG Screenshot}
Saves a screenshot directly in PNG format without external dependencies.

\subsubsection{\label{Record 1 Cycle (MP4)}Record 1 Cycle (MP4)}
Records one complete harmonic or sequence cycle to \texttt{movie\_cycle.mp4} matching the on-screen animation timing (requires \texttt{ffmpeg}).

\subsubsection{\label{Record 1 Cycle (GIF)}Record 1 Cycle (GIF)}
Records one complete harmonic or sequence cycle to \texttt{movie\_cycle.gif} (no external dependencies).

\subsubsection{\label{Record Continuous MP4 Video}Record Continuous MP4 Video}
Records animation frames continuously to \texttt{movie.mp4} (requires \texttt{ffmpeg}).

\subsubsection{\label{Record Continuous GIF Movie}Record Continuous GIF Movie}
Records animation frames continuously to an animated \texttt{movie.gif} file (no external dependencies).

\subsubsection{\label{Stop Recording}Stop Recording}
Finalizes and closes the active video or GIF movie recording.
...
\begin{verbatim}
   'cmap' 'cubehelix'|'coolwarm'|'turbo'|'viridis'|
          'inferno'|'jet'|'classic'|'gray'
\end{verbatim}
Changes the colormap of the color scale\footnote{Extended colormap palettes and \texttt{cubehelix} default palette introduced in CalculiX GraphiX (GLFW Edition).}. In the GLFW Edition, the default palette is \texttt{cubehelix} with a lifted graphite floor for optimal 3D metallic surface shading and black-and-white printing clarity. Available palettes include \texttt{cubehelix}, \texttt{coolwarm}, \texttt{turbo}, \texttt{viridis}, \texttt{inferno}, \texttt{jet}, \texttt{classic}, and \texttt{gray}. The user might use this command to store his personal setting in the ``.cgx'' file in his home directory. See also ''\htmlref{steps}{steps}'' to adjust the nr of boxes.
...
\begin{figure}[h]
\includegraphics[width=9cm]{corrad.pdf}
...
\begin{verbatim}
   'hcpy' ['png'|'name']|
          [clean]
\end{verbatim}
In CalculiX GraphiX (GLFW Edition)\footnote{In CalculiX GraphiX (GLFW Edition), screenshot captures are written directly to PNG format without requiring ImageMagick.}, this keyword saves a screenshot directly in PNG format without external dependencies.\\\\hcpy\\\\will create a PNG file with the default name hcpy\_\verb_<_nr\verb_>_.png\\\\hcpy my\_name\\\\will create my\_name.png\\\\hcpy clean\\\\will remove all session ''hcpy\_*.png'' files. See also the commands ''\htmlref{ulin}{ulin}'', ''\htmlref{rot}{rot}'', ''\htmlref{ds}{ds}'', ''\htmlref{max}{max}'', and ''\htmlref{min}{min}'' which might be used in hcpy batch files.
...
\subsection{\label{movi}movi / movie}
\begin{verbatim}
   'movie' [cycle [filename]]|[delay <sec>]|[start [filename]]|
           [stop]|[frames <nr>|'auto' [filename]]|[clean]
\end{verbatim}
In CalculiX GraphiX (GLFW Edition)\footnote{In CalculiX GraphiX (GLFW Edition), movie recording writes directly to MP4, GIF, or OGV without requiring ImageMagick, and supports 1-cycle animation recording.}, this keyword is used to record animation videos or animated GIFs. Supported formats are MP4 (\texttt{.mp4}, default, requires \texttt{ffmpeg}), animated GIF (\texttt{.gif}, zero external dependencies), and OGV (\texttt{.ogv}, requires \texttt{ffmpeg}).

Use 'movie cycle [name.mp4|name.gif]' to record one complete harmonic cycle or dataset sequence period matching the on-screen animation timing. Use 'movie start [name.mp4]' to begin continuous recording, and 'movie stop' to finish. Use 'movie frames <N> [name.mp4]' to record a specified number of frames. Use 'movie delay <sec>' to adjust the frame delay. Use 'movie clean' to remove session movie output files.

See \texttt{exporting\_videos.md} for additional details and platform instructions. 
...
\begin{figure}[h]
\includegraphics[width=12cm]{qcut.pdf}
...
\begin{figure}[h]
\includegraphics[width=12cm]{coords.pdf}
...
\begin{figure}[h]
\includegraphics[width=12cm]{qfil.pdf}
...
\begin{figure}[h]
\includegraphics[width=12cm]{qtxt.pdf}
...
\begin{verbatim}
   'rot' ['n' <set>|'nodenr'] |
         [['u'|'d'|'r'|'l'|'c' <angle>]|->
          ...
          ['iso'|'dim'|'trim']]
\end{verbatim}
This keyword is used to rotate the view-direction to the model. For example\\\\rot u 10\\\\will rotate the view-direction 10 degrees upwards. The meaning of the other letters is down d, right r, left l, clockwise c. The remaining parameters will rotate the view into a pre-defined direction. For example the user will look in x direction after the command\\\\rot x\\\\and against the x direction with the command\\\\rot -x\\\\In CalculiX GraphiX (GLFW Edition)\footnote{Axonometric projection viewpoints (Isometric, Dimetric, Trimetric) introduced in CalculiX GraphiX (GLFW Edition).}, the parameters ''iso'' (or ''isometric''), ''dim'' (or ''dimetric''), and ''trim'' (or ''trimetric'') rotate the model into standard axonometric projections.
...
The VTK XML UnstructuredGrid format (vtu) is supported for exporting meshes and result fields directly to modern scientific visualizers like ParaView \cite{paraview}\footnote{Native Base64 binary / ASCII VTU exporter and ParaView collection (.pvd) time-history generation introduced in CalculiX GraphiX (GLFW Edition).}:
\begin{itemize}
\item \texttt{send <set> vtu}: Exports the specified set (e.g. \texttt{all}) and the currently active dataset in high-performance Base64 inline binary encoding into a \texttt{<set>.vtu} file.
\item \texttt{send <set> vtu ds<nr>}: Exports the mesh with a specific dataset number (e.g. \texttt{send all vtu ds2}).
\item \texttt{send <set> vtu all}: Iterates through all available time-steps or modal shapes, writes individual timestamped \texttt{<set>\_<nr>.vtu} files, and automatically generates a master ParaView Collection file (\texttt{<set>.pvd}) for 1-click animated time-history playback in ParaView.
\item \texttt{send <set> vtu [all|ds<nr>] ascii}: Appending the optional \texttt{ascii} keyword forces the XML file to be written in human-readable plain text rather than binary encoding.
\end{itemize}
All 1D (BEAM2, BEAM3), 2D (TR3, TR6, QUAD4, QUAD8), and 3D (TET4, TET10, HEX8, HEX20, WEDGE6, WEDGE15, PYR5, PYR13) elements are automatically converted to their exact canonical VTK cell geometries with correct mid-side node permutations. See also ''\htmlref{How to export results to ParaView}{How to export results to ParaView}''.
...
\begin{figure}[h]
\includegraphics[width=12cm]{seta.pdf}
...
\begin{figure}[h]
\includegraphics[width=12cm]{shpe.pdf}
...
\item ''line'' Elements are displayed as wireframes (rendered with anti-aliased smoothing \texttt{GL\_LINE\_SMOOTH} and Retina auto-scaling\footnote{Retina / High-DPI line and point smoothing and auto-scaling introduced in CalculiX GraphiX (GLFW Edition).}).
\item ''point'' Element-edges are displayed as points with a pixel-width of 'value' (rendered with anti-aliased circular points \texttt{GL\_POINT\_SMOOTH}).
...
\item ''persp'' and ''ortho'' switch between realistic 3D perspective projection\footnote{3D perspective projection mode by default with center-anchored framing introduced in CalculiX GraphiX (GLFW Edition).} and classic orthographic (parallel) projection.
\item ''dark'' and ''light'' switch the viewport theme between modern high-contrast dark slate (\texttt{\#0D121A})\footnote{Signature Dark Mode theme by default introduced in CalculiX GraphiX (GLFW Edition).} and classic clean white background.
\end{itemize}  
...
%
\put(0,0){\includegraphics[width=9cm]{be2.pdf} }
...
%
\put(0,360){\includegraphics[width=9cm]{be3.pdf} }
\put(100,330){\label{topo_be3} 3 node beam element (be3, type 12) }
%
\put(0,30){\includegraphics[width=10cm]{tr3.pdf} }
...
\begin{picture}(90,180)(0,330)
%
\put(0,360){\includegraphics[width=10cm]{tr6.pdf} }
\put(100,330){\label{topo_tr6} 6 node shell element (tr6, type 8) }
%
\put(0,30){\includegraphics[width=10cm]{qu4.pdf} }
...
\begin{picture}(90,180)(0,330)
%
\put(0,360){\includegraphics[width=10cm]{qu8.pdf} }
\put(100,330){\label{topo_qu8} 8 node shell element (qu8, type 10) }
%
\put(0,30){\includegraphics[width=10cm]{te4.pdf} }
...
\begin{picture}(90,180)(0,330)
%
\put(0,310){\includegraphics[width=10cm]{te10.pdf} }
\put(100,280){\label{topo_te10} 10 node tet element (type 6) }
%
\put(0,30){\includegraphics[width=10cm]{he8.pdf} }
...
\begin{picture}(90,180)(0,330)
%
\put(0,360){\includegraphics[width=10cm]{he20.pdf} }
\put(100,330){\label{topo_he20} 20 node brick element (he20, type 4) }
%
\put(0,30){\includegraphics[width=6cm]{pe6.pdf} }
...
\begin{picture}(90,180)(0,330)
%
\put(0,360){\includegraphics[width=6cm]{pe15.pdf} }
...
\begin{figure}[h]
\includegraphics[width=12cm]{unconnectedBodies.pdf}
...
\begin{figure}[h]
\includegraphics[width=12cm]{equation.pdf}
...
\subsection{\label{How to export results to ParaView}How to export meshes and results to ParaView (VTU/PVD)}
\footnote{Native VTK XML UnstructuredGrid and ParaView collection (.pvd) exporter introduced in CalculiX GraphiX (GLFW Edition).}
The VTU export functionality enables direct visualization and post-processing of CalculiX FE models in ParaView \cite{paraview}. It converts 1D, 2D, and 3D linear and quadratic elements (TET4, TET10, HEX8, HEX20, WEDGE6, WEDGE15, PYR5, PYR13, TR3, TR6, QUAD4, QUAD8, BEAM2, BEAM3) to canonical VTK cells, preserving high-order node numberings and field data.

\subsubsection{Exporting Single Result Datasets}
To export the mesh along with the currently active dataset (e.g. von Mises stress or displacements):
\begin{verbatim}
  cgx model.frd
  ds 1 e 4
  send all vtu
\end{verbatim}
This produces \texttt{all.vtu} containing the mesh coordinates, element topologies, and the active scalar/vector result fields encoded in high-performance Base64 inline binary format. If you need human-readable plain text (for inspection or debugging), append \texttt{ascii}:
\begin{verbatim}
  send all vtu ascii
\end{verbatim}

\subsubsection{Exporting Multi-Step Transient or Modal Animations}
To export an entire simulation sequence (all time steps from a dynamic/transient calculation or all vibration mode shapes from a frequency analysis):
\begin{verbatim}
  send all vtu all
\end{verbatim}
This command automatically:
\begin{enumerate}
\item Iterates through all available datasets in the database.
\item Writes individual time-step files: \texttt{all\_1.vtu}, \texttt{all\_2.vtu}, \dots, \texttt{all\_N.vtu}.
\item Generates a master ParaView Collection file: \texttt{all.pvd} linking each VTU file with its exact physical time or frequency.
\end{enumerate}

\subsubsection{Visualizing in ParaView}
To open and inspect the exported results:
\begin{enumerate}
\item Launch ParaView and open the generated \texttt{all.pvd} or \texttt{all.vtu} file.
\item In the Properties panel, click \textbf{Apply}.
\item In the Active Variable Controls toolbar, select your desired result field (e.g. \texttt{DISP} or \texttt{STRESS}).
\item To view the deformed shape, apply the \textbf{Warp By Vector} filter with the displacement vector field (\texttt{DISP}) and set an appropriate scale factor.
\item For transient/modal datasets, click the \textbf{Play} button in the VCR animation toolbar to playback the time-history sequence smoothly.
\end{enumerate}
...
\item The CAD format is supported by a stand alone translator (see the calculix home pages). In this case the user can mesh the surfaces with structured- \cite{Coons} (elty setname tr3 etc.) or unstructured triangles \cite{mesh2d} (elty setname tr3u etc.). The user might modify this surface mesh until he is happy with it. The surface mesh can then be filled with tets created by an auto-mesher called from within cgx (mesh setname tet). The cgx uses the tet-mesher from TETGEN \cite{TETGEN} (embedded in-memory in the GLFW Edition\footnote{In CalculiX GraphiX (GLFW Edition), TetGen 1.6.0 is directly embedded into the binary with an in-memory C++ interface and default Delaunay quality switches \texttt{pq1.3/17O7}, requiring no external binaries or disk I/O.}) or NETGEN \cite{NETGEN}. The command ''\htmlref{asgn}{asgn}'' is used to switch between them.

In the GLFW Edition, TetGen 1.6.0 is fully built-in and active out of the box with zero external installation required. When using NETGEN, replace the original ng\_vol source file in the NETGEN package with the modified program ng\_vol from the cgx-distribution and build it again. This version regards a target element size. Tetgen is able to regard that size unchanged.
...
\begin{figure}[h]
\includegraphics[width=9cm]{halter.pdf}
...
\begin{figure}[h]
\includegraphics[width=12cm]{surfmesh.pdf}
...
\end{verbatim}
  ...
  \begin{figure}[h]
\includegraphics[width=9cm]{scratch.pdf}
...
\begin{figure}[h]
\includegraphics[width=12cm]{disc.pdf}
...
\begin{figure}[h]
\includegraphics[width=12cm]{cylinder.pdf}
...
\begin{figure}[h]
\includegraphics[width=12cm]{sphere.pdf}
...
\begin{figure}[h]
\includegraphics[width=12cm]{sphere_vol.pdf}
...
\begin{figure}[h]
\includegraphics[width=12cm]{sphere_thinvol.pdf}
...
\begin{figure}[h]
\includegraphics[width=12cm]{naca23012.pdf}
...
\begin{figure}[h]
\includegraphics[width=12cm]{ifwhile.pdf}
...
\bibitem{glfw} GLFW - An Open Source, Multi-Platform Library for OpenGL, https://www.glfw.org/
\bibitem{stb} stb\_truetype - Single-header vector font engine from Sean Barrett, https://github.com/nothings/stb
\bibitem{paraview} ParaView - Open-source, multi-platform data analysis and visualization application, Kitware, https://www.paraview.org/
\bibitem{cubehelix} D. A. Green, 'A colour scheme for the display of astronomical intensity images', Bulletin of the Astronomical Society of India, Vol. 39, pp. 289-295 (2011), https://astron-soc.in/bulletin/11June/289392011.pdf
