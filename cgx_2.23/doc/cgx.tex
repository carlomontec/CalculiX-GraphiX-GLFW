...
Changes the colormap of the color scale. In the GLFW Edition\footnote{Perceptually uniform \texttt{cubehelix} default colormap with lifted graphite floor and modern palettes introduced in CalculiX GraphiX (GLFW Edition).}, the default colormap is \texttt{cubehelix} (optimized for metallic 3D surface shading and grayscale print clarity). Available palettes include \texttt{cubehelix}, \texttt{coolwarm}, \texttt{turbo}, \texttt{viridis}, \texttt{inferno}, \texttt{jet}, \texttt{classic}, and \texttt{gray}. See also ''\htmlref{cmap}{cmap}''.
