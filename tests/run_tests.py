...
TEST_EXPECTATIONS: Dict[str, Dict] = {
    ...
    "test_post_node_history_graph": {
        "files": ["graph_0.gnu", "graph_0.out", "graph_0.py"],
        "file_contains": {
            "graph_0.gnu": ["set term pngcairo", "plot \"graph_0.out\""],
            "graph_0.py": ["matplotlib", "plt.savefig"],
        }
    }
}
...
def validate_test_artifacts(test_name: str, run_dir: Path) -> Tuple[bool, Optional[str]]:
    ...
    # If gnuplot is installed, verify rendered PNG output
    if test_name == "test_post_node_history_graph" and shutil.which("gnuplot"):
        png_path = run_dir / "graph_0.png"
        if not png_path.exists() or png_path.stat().st_size == 0:
            return False, "Rendered plot 'graph_0.png' was not generated by gnuplot."
